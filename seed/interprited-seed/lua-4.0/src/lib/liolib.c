...
static int io_tmpname (lua_State *L) {
  #define MY_TMP_FILE_SIZE 32
  static char static_buffer[MY_TMP_FILE_SIZE];
  snprintf(static_buffer, MY_TMP_FILE_SIZE , "/tmp/tmpfile_%ld_%d", time(NULL), getpid());
  lua_pushstring(L, static_buffer);
  return 1;
}
