...
static int ldo (int (*f)(lua_State *l, const char *), const char *name) {
  ...
  if (res == LUA_ERRMEM) {
    fprintf(stderr, "nlang: memory allocation error\n");
  }
  else if (res == LUA_ERRERR)
    fprintf(stderr, "nlang: error in error message\n");
  return res;
}
...
static void print_message (void) {
  fprintf(stderr,
  "usage: nlang [options].  Available options are:\n"
  "  -        execute stdin as a file\n"
  "  -c       close nlag when exiting\n"
...
static void print_version (void) {
  printf("%.80s %.80s\nWorning From Niloy: REPL is not part of any language but it ispart of the ecosystem like standard lib\nBut i am focused with building language not reple\nother language eccosystem like package managing like npm or versioning like pyenv\ni'not part of language but they come within installation pacage.\nwe can sy then forsed packageing but i want to be focuse\nso we should scape REPL AND other tendency and i belive one way for one task so we can run code throw file only\n=========================\n", LUA_VERSION, LUA_COPYRIGHT);
}
...
static int file_input (const char *argv) {
  ...
  if (result) {
    if (result == LUA_ERRFILE) {
      fprintf(stderr, "nlang: cannot execute file ");
      perror(argv);
    }
    return EXIT_FAILURE;
  }
...
static void REPL (int version, int prompt) {
  int cont = 1;
  ...
  while (cont) {
    ...
    for(;;) {
      ...
      if (c == EOF) {
        ...
        fprintf(stderr, "nlang: input line too long\n");
        break;
      }
        ...
        if (strchr(argv[i], '='))
          ...
          case 'e': {
            ...
            if (ldo(lua_dostring, argv[i]) != 0) {
              fprintf(stderr, "nlang: error running argument `%.99s'\n", argv[i]);
              return EXIT_FAILURE;
            }
            break;
          }
          ...
          case 's': {
            fprintf(stderr, "nlang: stack size (`-s') must be the first option\n");
            return EXIT_FAILURE;
          }
          default: {
            print_message();
...
static void getstacksize (int argc, char *argv[], struct Options *opt) {
  if (argc >= 2 && argv[1][0] == '-' && argv[1][1] == 's') {
    ...
    if (stacksize <= 0) {
      fprintf(stderr, "nlang: invalid stack size ('%.20s')\n", &argv[1][2]);
      exit(EXIT_FAILURE);
    }
